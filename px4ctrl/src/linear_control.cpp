    ...
    Controller_Output_t &u)
{
  ...
  //     // Desired_State_t pc;
  //     // Gain & gain=
  //     des_acc(0) = des.a(0) + gain.Kv0*(des.v(0) - odom.v(0)) + gain.Kp0*(des.p(0) - odom.p(0));
  //     des_acc(1) = des.a(1) + gain.Kv1*(des.v(1) - odom.v(1)) + gain.Kp1*(des.p(1) - odom.p(1));
  //     des_acc(2) = des.a(2) + gain.Kv2*(des.v(2) - odom.v(2)) + gain.Kp2*(des.p(2) - odom.p(2));
  ...
  // Eigen::Vector3d des_euler = odom.q.toRotationMatrix().eulerAngles(2, 1, 0);


  // double phi_c, theta_c;
  
  // // u.thrust = mass_ * (g_ + pc.a(2));
  // phi_c = 1 / g_ * (des_acc(0) * sin(des_euler[0]) - des_acc(1) * cos(des_euler[0]));
  // theta_c = 1 / g_ * (des_acc(0) * cos(des_euler[0]) + des_acc(1) * sin(des_euler[0]));

  // Eigen::Vector3d EulerAngle{des_euler[0],theta_c, phi_c };
 ...
 /* WRITE YOUR CODE HERE */
      ...
      des_acc[2] = param_.gra + des.a(2) + param_.gain.Kv2*(des.v(2)-odom.v(2)) + param_.gain.Kp2*(des.p(2)-odom.p(2));
      ...
      Eigen::Vector3d des_euler = des.q.toRotationMatrix().eulerAngles(2,1,0);
      ...
      Eigen::Matrix3d R_E_B1 = R_E_B * R_W_B.transpose() * R_W_B1;
